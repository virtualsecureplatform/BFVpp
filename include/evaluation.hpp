...
template <class P>
inline void Decomposition(DecomposedPolynomial<P> &decvec, const Polynomial<P> &poly)
{
    ...
    for (int j = 0; j < P::n; j++) {
        ...
        for (int i = 0; i < P::l; i++)
            decvec[i][j] = ((temp >> (std::numeric_limits<typename P::T>::digits -
                                       (i + 1) * P::Bgbit)) &
                            mask) -
                           halfBg;
    }
...
template <class P>
inline void relinKeySwitch(Ctxt<P> &res, const Polynomial<P> &poly, const relinKeyFFT<P> &relinkeyfft){
...
template <class P>
inline void Relinearization(Ctxt<P> &res, const CtxtMult<P> &mult, const relinKeyFFT<P> &relinkeyfft){
    Ctxt<P> squareterm;
...
template <class P>
inline void Multiplication(Ctxt<P> &res, const Ctxt<P> &a, const Ctxt<P> &b, const relinKeyFFT<P> &relinkeyfft){
    CtxtMult<P> resmult;
